...
v0.8.0 — `inflect()`, `paradigm()`, `list_lemmas()`, `get_slot_templates()`, and `analyze()` for verbs, nouns, adjectives, and pronouns.
...
**v0.8.0** — Added pluperfect (`Tense: "Pqp"`, TVM key prefix `"Y"`) to the verb
paradigm: `_VERB_TENSES` now includes `"Y"` (was hardcoded to `"PIAFX"`, silently
excluding pluperfect from every `.paradigm()` call regardless of lexicon data),
and `verb-tags.tsv` gained 6 pluperfect active indicative rows (`YAI.1S`
through `YAI.3P` — active-only, same curation as the existing perfect/`XAI`
rows; no dual pluperfect stemming coverage exists, confirmed empirically).
`ag_verb_key`'s `Tense: "Pqp"` → `"Y"` mapping already existed and worked via
`.inflect()`; the gap was entirely in `.paradigm()`'s enumeration and in
`get_tags`/`get_slot_templates` never offering a pluperfect slot to query in
the first place. Fixes a real bug: `odyssey_morpheus_verbs_lexicon`'s `ἄνωγα`/
`ὄρνυμι` pluperfect `forms:` overrides were already correct but permanently
unreachable. A full-corpus scan (5052 verb lemmas) found the fix safe (no new
crashes) and confirmed pluperfect is mechanically generated by the stemming
engine for 119 lemmas beyond the two override cases (e.g. `λύω`, `βάλλω`).
